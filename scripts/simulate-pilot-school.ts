  ...
  try {
    ...
    try {
        classroomData = await generateFullClassroom(
            topic,
            (progress) => {
              console.log(`   [Study Arena] ${progress.step}: ${progress.progress}% - ${progress.message}`);
            },
            new AbortController().signal
        );
        durationClassroom = Date.now() - startClassroom;
        console.log(`✅ Classroom generated in ${durationClassroom}ms with ${classroomData.scenes.length} scenes.\n`);
    } catch (e: any) {
           ...
           scenes: [
             {
               ...
               title: "Introduction",
               events: [
                 { type: "dialogue", speaker: "Professor AI", content: `Welcome to the class on ${topic}.` }
               ]
             }
           ]
         };
      ...
      questions: [
        {
          ...
          maxPoints: 10,
          rubric: "1. Mentions a^2 + b^2 = c^2 (4 points). 2. Mentions right-angled triangles (3 points). 3. Explains finding unknown side (3 points).",
          studentAnswer: "The Pythagorean theorem is a^2 + b^2 = c^2. It is used to find the length of the hypotenuse in a right triangle if you know the other two sides."
        }
      ]
    };
    ...
    try {
        ...
        console.log(`✅ Grading completed in ${durationGrading}ms. Score: ${gradingResult.scoreBreakdown.totalScore}\n`);
