...
export default function EducatorDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageHeader title="Educator Dashboard" subtitle="Manage your classes and track student progress" />
      <div className="grid gap-4 md:grid-cols-4 p-4">
        ...
        <Card>
          ...
          <CardContent><div className="text-2xl font-bold">{stats?.totalTests || 0}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent><div className="text-2xl font-bold">{stats?.pendingGrading || 0}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent><div className="text-2xl font-bold">{stats?.avgScore || 0}%</div></CardContent>
        </Card>
      </div>
      <div className="p-4">
          ...
          {(recentTests || []).map((t: any) => (
            <div key={t.id} className="flex items-center justify-between p-4 border rounded">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t.subject} • {t.class}</p>
              </div>
