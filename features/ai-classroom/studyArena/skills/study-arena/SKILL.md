...
- Do not assume the FridayLearning agent's own model or API key will be reused by Friday Learning.
- Friday Learning classroom generation uses Friday Learning server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Friday Learning server-side config files may control provider selection and defaults.
...
If present, read defaults from `~/.friday-learning/friday-learning.json` under:
...
If no `accessCode` in config, ask the user how they want to use Friday Learning:

1. **Use hosted Friday Learning** (recommended for quick start) — Requires an access code from open.maic.chat. No local setup needed.
...
Use this when the user has not installed Friday Learning yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Friday Learning

After the user has chosen a startup mode and configured keys, start Friday Learning using the chosen method, then verify the service with `GET {url}/api/health`.
