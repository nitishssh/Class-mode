...
1. Read `accessCode` from skill config (`~/.friday-learning/friday-learning.json` → `skills.entries.friday-learning.config.accessCode`).
...
3. If not found, tell the user to add their access code to the config file:
   ```
   Edit ~/.friday-learning/friday-learning.json and set skills.entries.friday-learning.config.accessCode to your access code (starts with sk-).
   ```
