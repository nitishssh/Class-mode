...
export function getSlidesForTableRows(
  ...
  function calcSlideTabH(): void {
    ...
    if (tableRowSlides.length > 0)
      emuStartY = inch2Emu(
        tableProps.autoPageSlideStartY || tableProps.newSlideStartY || arrInchMargins[0],
      );
    ...
    if (tableRowSlides.length > 1) {
      ...
      if (typeof tableProps.autoPageSlideStartY === 'number') {
        ...
        emuSlideTabH =
          (tablePropH || presLayout.height) -
          inch2Emu(
            (tablePropY / EMU < arrInchMargins[0] ? tablePropY / EMU : arrInchMargins[0]) +
              arrInchMargins[2],
          );
  ...
  if (tableProps.verbose) {
    ...
    console.log(
      `| presLayout.width ................................ = ${(presLayout.width / EMU).toFixed(1)}`,
    );
    console.log(
      `| presLayout.height ............................... = ${(presLayout.height / EMU).toFixed(1)}`,
    );
    console.log(
    ...
    console.log(
      `| tableProps.margin ............................... = ${String(tableProps.margin)}`,
    );
  ...
  // STEP 1: Calculate margins
  {
    ...
    if (tableProps.verbose)
      console.log(
        `| arrInchMargins .................................. = [${arrInchMargins.join(', ')}]`,
      );
  }
  ...
  // STEP 4: Calculate usable width now that total usable space is known (`emuSlideTabW`)
  {
    emuSlideTabW =
      tableCalcW ||
      inch2Emu((tablePropX ? tablePropX / EMU : arrInchMargins[1]) + arrInchMargins[3]);
    if (tableProps.verbose)
      console.log(
        `| emuSlideTabW .................................... = ${(emuSlideTabW / EMU).toFixed(1)}`,
      );
  }
  ...
  tableRows.forEach((row, iRow) => {
    ...
    if (tableProps.verbose && iRow === 0)
      console.log(
        `| SLIDE [${tableRowSlides.length}]: emuSlideTabH ...... = ${(emuSlideTabH / EMU).toFixed(1)} `,
      );
    ...
    row.forEach((cell, iCell) => {
        ...
        _lineHeight: inch2Emu(
          ((cell.options?.fontSize
            ...
            : tableProps.fontSize
              ...
              : DEF_FONT_SIZE) *
            (LINEH_MODIFIER +
              (tableProps.autoPageLineWeight ? tableProps.autoPageLineWeight : 0))) /
            100,
        ),
        text: [],
        options: cell.options,
      ...
      if (cell.options.colspan && Array.isArray(tableProps.colW)) {
        totalColW = tableProps.colW
          .filter((_cell, idx) => idx >= iCell && idx < idx + cell.options.colspan)
    ...
    while (!isDone) {
      ...
      if (emuTabCurrH + emuLineMaxH > emuSlideTabH) {
        ...
        if (
          ...
          currTableRow.map((cell) => cell.text.length).reduce((p, n) => p + n) > 0
        )
      ...
      // 7: WIP: done?
      const brent = rowCellLines
        .map((cell) => cell._lines.length)
...
export function genTableToSlides(
  ...
  if (opts.verbose) {
    ...
    console.log(
      `| pptx.presLayout.width ........................... = ${(pptx.presLayout.width / EMU).toFixed(1)}`,
    );
    console.log(
      `| pptx.presLayout.height .......................... = ${(pptx.presLayout.height / EMU).toFixed(1)}`,
    );
    console.log(
      `| emuSlideTabW .................................... = ${(emuSlideTabW / EMU).toFixed(1)}`,
    );
  ...
  arrTabColW.forEach((colW, idxW) => {
    const intCalcWidth = Number(
      ((Number(emuSlideTabW) * ((colW / intTabW) * 100)) / 100 / EMU).toFixed(2),
    );
    let intMinWidth = 0;
    const colSelectorMin = document.querySelector(
      `#${tabEleId} thead tr:first-child th:nth-child(${idxW + 1})`,
    );
    if (colSelectorMin) intMinWidth = Number(colSelectorMin.getAttribute('data-pptx-min-width'));
    const intSetWidth = 0;
    const colSelectorSet = document.querySelector(
      `#${tabEleId} thead tr:first-child th:nth-child(${idxW + 1})`,
    );
    ...
    document.querySelectorAll(`#${tabEleId} ${part} tr`).forEach((row: Element) => {
      ...
      Array.from(htmlRow.cells).forEach((cell) => {
        ...
        const cellOpts: TableCellProps = {
          ...
          fontSize: Number(
            window.getComputedStyle(cell).getPropertyValue('font-size').replace(/[a-z]/gi, ''),
          ),
          margin: null,
        ...
        if (
          ['top', 'middle', 'bottom'].includes(
            window.getComputedStyle(cell).getPropertyValue('vertical-align'),
          )
        ) {
