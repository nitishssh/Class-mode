...
class SearchPageManager {
  ...
  renderFilterInterface() {
    ...
    const tagOptions = tags
      .map((tag) => `<option value="${this.escapeHtml(tag)}">${this.escapeHtml(tag)}</option>`)
      .join("");
  ...
  getActiveFilterCount() {
    ...
    if (this.currentFilters.facets) {
      Object.values(this.currentFilters.facets).forEach((v) => {
        if (v) count++;
      });
    }
    return count;
    ...
    return Object.entries(facets)
      .map(([facetKey, facetValues]) => {
        ...
        const options = facetValues
          .map((value) => `<option value="${value}">${this.formatFacetValue(value)}</option>`)
          .join("");
   ...
   */
  clearFocus() {
    this.resultsContainer.querySelectorAll(".search-result.focused").forEach((el) => {
  ...
  renderMetadataBadges(result) {
    ...
    if (audienceField) {
      ...
      audienceList.forEach((audience) => {
        ...
        badges.push(
          `<span class="metadata-badge persona-badge" title="${this.escapeHtml(formatted)}">👤 ${this.escapeHtml(formatted)}</span>`
        );
      });
    }
    ...
    if (result.difficulty) {
      ...
      badges.push(
        `<span class="metadata-badge difficulty-badge" title="${this.formatDifficultyName(result.difficulty)}">${difficultyIcon} ${this.formatDifficultyName(result.difficulty)}</span>`
      );
    }
      ...
      Object.entries(result.facets).forEach(([facetKey, facetValue]) => {
        if (facetValue) {
          ...
          values.forEach((value) => {
            badges.push(
              `<span class="metadata-badge facet-badge" title="${this.formatFacetName(facetKey)}: ${this.formatFacetValue(value)}">🏷️ ${this.formatFacetValue(value)}</span>`
            );
          });
        }
      });
    ...
    if (result.modality && (!result.facets || !result.facets.modality)) {
      ...
      badges.push(
        `<span class="metadata-badge modality-badge" title="${this.formatFacetValue(result.modality)}">${modalityIcon} ${this.formatFacetValue(result.modality)}</span>`
      );
    }
  ...
  renderMatchingSections(result, query) {
    ...
    const sectionsHtml = sectionsToShow
      .map((section) => {
  ...
  showNoResults() {
    const filtersActive =
      this.currentFilters.topic ||
      this.currentFilters.category ||
      this.currentFilters.tag ||
      this.currentFilters.type ||
      (this.currentFilters.facets &&
        Object.keys(this.currentFilters.facets).some((k) => this.currentFilters.facets[k]));
