    ...
    Object.values(this.documents).forEach((doc) => {
      ...
      if (doc.tags) {
        if (Array.isArray(doc.tags)) {
          ...
          const allTags = doc.tags.includes(",") ? doc.tags.split(",") : doc.tags.split(" ");
  ...
  groupResultsByDocument(results, query) {
    ...
    results.forEach((result) => {
      ...
      if (!grouped.has(docId)) {
        ...
        existing.matchingSections = this.mergeSections(
          existing.matchingSections,
          additionalSections
        );
    ...
    return Array.from(grouped.values()).sort((a, b) => b.combinedScore - a.combinedScore);
  }
  ...
  mergeSections(existing, additional) {
    ...
    additional.forEach((section) => {
      const isDuplicate = existing.some(
        (existingSection) =>
          existingSection.text === section.text && existingSection.type === section.type
      );
