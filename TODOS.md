...
- [ ] **P3 — pending state on `/absentees` CSV download buttons**: `downloadCsv` is async with no disabled/spinner state — slow connections invite double-clicks. Mirror the `saveMutation.isPending` pattern from attendance.tsx.
