...
- [ ] **P3 — `school_admin` is excluded from every Study Arena surface**: `client/src/App.tsx:209-211` gates the three routes on `student`/`teacher`/`admin`; `school_admin` is in none (note `admin` ≠ `school_admin` here), so a principal hits "Access Denied" on `/study-arena-beta`, `/study-arena-report`, and `/study-arena/create`. Flagged, not changed — the intended principal permission model is a product call.
