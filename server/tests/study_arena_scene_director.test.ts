...
describe("Study Arena scene director", () => {
  it("starts from the declared entry and only follows a matching declared edge", () => {
    expect(
      resolveNextScene(script, { currentSceneId: null, branchPath: [] }, emptyEvidence)
    ).toMatchObject({
      toSceneId: "check",
    ...
    expect(
      resolveNextScene(
        script,
        { currentSceneId: "check", branchPath: ["check"] },
        {
          ...emptyEvidence,
  ...
  it("uses authored order deterministically and completes at a declared terminal edge", () => {
    ...
    expect(
      resolveNextScene(
        script,
        { currentSceneId: "done", branchPath: ["check", "done"] },
        emptyEvidence
      )
        ...
        transitions: [
          {
            fromSceneId: "check",
            toSceneId: "support",
            when: { kind: "help_depth", operator: "gte", value: 3 },
          },
          {
            fromSceneId: "check",
            toSceneId: "support",
            when: {
              kind: "prerequisite_mastery",
              conceptId: "fractions",
              operator: "lt",
              value: 0.5,
            },
  ...
  it("retains implicit linear progression for legacy scripts", () => {
    ...
    expect(
      resolveNextScene(legacy, { currentSceneId: "check", branchPath: ["check"] }, emptyEvidence)
