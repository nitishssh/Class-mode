...
vi.mock("../lib/pg-queries", () => ({
  pgGetTimetableByWorkspace: vi.fn().mockResolvedValue([{ id: 1, subject: "Math" }]),
  pgGetTimetableByClass: vi.fn().mockResolvedValue([{ id: 2, subject: "Science" }]),
  pgCreateTimetableSlot: vi.fn().mockResolvedValue({ id: 3, subject: "History" }),
...
describe("Timetable Routes", () => {
  ...
  describe("GET /api/timetable", () => {
    ...
    it("returns timetable for workspace", async () => {
      ...
      expect(res.body).toEqual([{ id: 1, subject: "Math" }]);
    });
  ...
  describe("GET /api/timetable/class/:className", () => {
    ...
    it("returns class timetable", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body).toEqual([{ id: 2, subject: "Science" }]);
    });
