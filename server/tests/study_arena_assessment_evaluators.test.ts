  ...
  it("requires grounded source coverage and a versioned evaluator", () => {
    expect(
      scienceTemplateSchema.safeParse({
        ...
        sourceSpans: [{ sourceLabel: "Textbook", excerpt: "Photosynthesis uses light energy to transform carbon dioxide and water into glucose." }],
      }).success
    ).toBe(true);
  });
  ...
  it("accepts an accessible semantic highlight action", () => {
    expect(
      lessonScriptSchema.safeParse({
        ...
        scenes: [{ id: "inputs", actions: [{ type: "highlight", target: "Leaf diagram", label: "Notice that carbon dioxide enters through the leaf." }, { type: "ask", agent: "teacher", prompt: "What enters the leaf?", expects: "freeText", gate: true }] }],
      }).success
    ).toBe(true);
  });

  it("evaluates the Science transfer check with a server-owned rubric", () => {
    expect(evaluateAssessment("photosynthesis-transfer", "Light and carbon dioxide help the plant make glucose.").correct).toBe(true);
    expect(evaluateAssessment("photosynthesis-transfer", "Plants make food from soil.").misconceptionCode).toBe("photosynthesis-input-output");
  });

  it("keeps adaptive recommendations server-policy bounded", () => {
    expect(getRecommendedNextAction({ correct: true, pMastery: 0.8, prerequisiteMastery: null, helpDepth: 1 })).toBe("schedule_recall");
    expect(getRecommendedNextAction({ correct: false, pMastery: 0.2, prerequisiteMastery: 0.2, helpDepth: 1 })).toBe("prerequisite_refresh");
  });
});
...
describe("Study Arena English and Social Studies templates", () => {
  const sourceSpans = [{
    ...
    excerpt: "Mira left her umbrella by the door, but dark clouds gathered before she walked home.",
  }];
  ...
  it("requires source evidence and server-only English inference targets", () => {
    ...
    expect(
      evaluateAssessment("english-reading-inference", "Mira may get wet.", template.evaluationTargets)
  ...
  it("evaluates Social Studies cause-and-effect without an LLM", () => {
    ...
    expect(
      evaluateAssessment("social-studies-causation", "There were high grain taxes.", template.evaluationTargets)
