...
const createAssignedLessonSchema = z
  .object({
  ...
  const result = await getPgPool().query<{
    ...
    `SELECT a.id, v.objective, v.subject, v.grade_level, a.due_at, s.status AS session_status
       ...
       LEFT JOIN study_arena_attempt_sessions s
         ON s.assignment_id = a.id AND s.student_id = e.student_id AND s.is_preview = false
...
router.post("/assignments", requireFlag, authenticateToken, async (req: Request, res: Response) => {
  ...
  if (template && parsed.data.subject.trim().toLowerCase() !== subjectForTemplate(template)) {
    return res.status(400).json({
      message: `${template.templateId} requires ${subjectForTemplate(template)} as the subject`,
    });
  }
  if (template) {
    ...
    if (!assessmentIds.includes(requiredAssessmentForTemplate(template))) {
      return res
        .status(400)
        .json({ message: "This subject template requires its verified transfer check" });
    }
  }
  ...
  if (!primaryConceptId || !getStudyArenaConcept(primaryConceptId)) {
    return res
      .status(400)
      .json({ message: "An assigned lesson requires a supported primary concept" });
  }
  ...
  try {
    ...
    const version = await client.query<{ id: string }>(
      ...
      [
        ...
        JSON.stringify(parsed.data.script),
        template?.evaluatorVersion ?? "v1",
        template?.templateId ?? null,
        JSON.stringify(
          template && "evaluationTargets" in template ? template.evaluationTargets : {}
        ),
        primaryConceptId,
      ]
    );
    ...
    return res
      .status(201)
      .json({ assignmentId: assignment.rows[0].id, lessonVersionId: version.rows[0].id });
  } catch (error) {
    await client.query("ROLLBACK").catch(() => {});
    logger.error("[study-arena-beta] assignment creation failed", {
      error: String(error),
      workspaceId,
      teacherId,
    });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!workspaceId) return res.status(409).json({ message: "No active workspace" });
    if (!isPgReady())
      return res.status(503).json({ message: "Drafts are temporarily unavailable" });
    ...
    if (parsed.data.compilerJobId) {
      ...
      return res
        .status(200)
        .json({ lessonVersionId: job.lessonVersionId, compilerJobId: job.id, reused: true });
    }
    ...
    if (!primaryConceptId || !getStudyArenaConcept(primaryConceptId)) {
      return res
        .status(400)
        .json({ message: "A draft lesson requires a supported primary concept" });
    }
    ...
    try {
      ...
      try {
        ...
        return res
          .status(201)
          .json({ lessonVersionId: version.rows[0].id, compilerJobId: linkedJobId });
      } catch (error) {
        await client.query("ROLLBACK").catch(() => {});
        throw error;
      } finally {
      ...
      logger.error("[study-arena-beta] lesson draft creation failed", {
        error: String(error),
        workspaceId,
        teacherId,
      });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!workspaceId) return res.status(409).json({ message: "No active workspace" });
    if (!isPgReady())
      return res.status(503).json({ message: "Approvals are temporarily unavailable" });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!workspaceId) return res.status(409).json({ message: "No active workspace" });
    if (!isPgReady())
      return res.status(503).json({ message: "Publish is temporarily unavailable" });
    ...
    try {
      ...
      if (enrolled.rows.length !== body.data.studentIds.length) {
        ...
        return res
          .status(400)
          .json({ message: "Every student must be an active workspace member" });
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (teacherId === null) return;
    const lessonVersionId = z.string().uuid().safeParse(req.params.id);
    if (!lessonVersionId.success)
      return res.status(400).json({ message: "Invalid lesson version ID" });
    const workspaceId = (req as any).workspace?.id as number | undefined;
    if (!workspaceId) return res.status(409).json({ message: "No active workspace" });
    ...
    try {
      ...
      if (result.status === "forbidden")
        return res.status(403).json({ message: "Not allowed to preview this lesson" });
      if (result.status === "database_unavailable") {
        return res.status(503).json({ message: "Preview is temporarily unavailable" });
      }
      if (result.status !== "ok")
        return res.status(409).json({ message: "Could not open preview" });
      return res.json({
        status: "ok",
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (result === "not_cancellable")
      return res.status(409).json({ message: "Job cannot be cancelled" });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (result.status === "not_retryable")
      return res.status(409).json({ message: "Job cannot be retried" });
    if (result.status === "database_unavailable")
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!workspaceId) return res.status(409).json({ message: "No active workspace" });
    if (!isPgReady())
      return res.status(503).json({ message: "Reports are temporarily unavailable" });
    try {
      ...
      const groups = [
        {
          ...
          studentIds: completed
            .filter((row) => row.assessment_correct === false)
            .map((row) => row.student_id),
          suggestedAction: "Review the misconception and assign supported practice.",
        },
        {
          key: "high_help",
          label: "High help depth",
          studentIds: report.rows.filter((row) => row.help_depth >= 3).map((row) => row.student_id),
          suggestedAction: "Check prerequisite understanding in a small group.",
        },
      ...
      logger.error("[study-arena-beta] assignment report failed", {
        error: String(error),
        assignmentId: assignmentId.data,
      });
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!assignmentId.success || !body.success)
      return res.status(400).json({ message: "Invalid intervention" });
    if (!workspaceId || !isPgReady())
      return res.status(503).json({ message: "Interventions are temporarily unavailable" });
    const teacherId = (req.user?.id || req.session?.userId) as number;
    const inserted = await getPgPool().query(
     ...
     RETURNING id`,
      [
        assignmentId.data,
        body.data.studentId ?? null,
        teacherId,
        body.data.cohortKey,
        body.data.actionNote,
        workspaceId,
      ]
    );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!canUseAttemptSession(req)) {
      return res
        .status(403)
        .json({ message: "Only assigned students or previewing teachers can start an assessment" });
    }
    ...
    try {
      ...
      if (
        issued.status === "inactive" ||
        issued.status === "out_of_sequence" ||
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (submitted.status === "database_unavailable")
        return res.status(503).json({ message: "Assessments are temporarily unavailable" });
      if (
        ...
        return res.status(409).json({ message: "This assessment submission is no longer valid" });
...
const interactionSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (
      assignedFields.some((field) => field !== undefined) &&
      assignedFields.some((field) => field === undefined)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    try {
      ...
      if (parsed.data.attemptSessionId && canUseAttemptSession(req)) {
        ...
        if (actionCheck.status === "inactive" || actionCheck.status === "out_of_sequence") {
          return res
            .status(409)
            .json({ message: "This lesson step is no longer available. Refresh to continue." });
        }
