...
test.describe("Master Timetable & Conflict Guard", () => {
  test("should authenticate, assign slot, detect conflicts, and delete slot", async ({ page }) => {
    ...
    await expect(page.locator("text=Class Conflict: Math 101 is already attending Mathematics")).toBeVisible();
    await expect(page.locator("text=Teacher Conflict: Jane Teacher is already scheduled to teach Math 101")).toBeVisible();
    await expect(page.locator("text=Room Conflict: Room Room 101 is already occupied by Math 101")).toBeVisible();
