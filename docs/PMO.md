...
| ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| Signup email dead in production (#322) — **ops half**   | Production still holds an invalid Resend key. Rotate → `SMTP_PASS` in Secret Manager → redeploy → re-test signup on prod. **Only the founder can do this**; it needs credential access. Open **39 days**.                                                                                                                                                                                            | OPEN P0, founder |
...
| `GET /api/chat/dms` → 404 (#324.2)                      | DONE 2026-08-18. The client asked for a path the router has never registered (`/users/me/dms`), so the DM list was empty for everyone with no error shown. Also fixed the blank-row bug behind it: conversations were titled with `username`, empty for every seeded and invited account. A contract test now checks every `chat-api.ts` path against the router's real route table.                 | SHIPPED          |
